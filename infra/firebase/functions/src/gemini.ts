...
export interface GeminiReadingResult {
  ...
  horoscope_match?: string; // Kratka horoskopska potvrda (1-2 rečenice, opcionalno)
}
...
function extractSymbols(result: GeminiReadingResult): string[] {
...
function shouldRetryForGenericSymbols(result: GeminiReadingResult, readingMode: string): boolean {
  // Deep mode is already verbose; don't add extra Gemini calls there by default.
  if (readingMode === "deep") return false;

  const symbols = extractSymbols(result);
  if (symbols.length === 0) return true;

  // Primary complaint: "Zmija" showing up too often.
  if (symbols.includes("zmija")) return true;
...
function parseGeminiResponse(text: string): GeminiReadingResult {
  ...
  try {
    ...
    let visibleSymbols: VisibleSymbol[] = [];
    if (Array.isArray(parsed.visible_symbols)) {
      visibleSymbols = parsed.visible_symbols
        ...
        .filter((s: VisibleSymbol | null): s is VisibleSymbol => s !== null);
    }
    ...
    const containsZodiac = (text: string): boolean => {
    ...
    return {
      ...
      symbols: Array.isArray(parsed.symbols) 
        ? parsed.symbols.slice(0, 5).map((s: any) => String(s))
        : visibleSymbols.map(s => s.symbol),
      lucky_numbers: Array.isArray(parsed.lucky_numbers)
        ? parsed.lucky_numbers.filter((n: any) => typeof n === "number" && n > 0 && n <= 49)
...
function getSafeFallbackResponse(reason?: string): GeminiReadingResult {
