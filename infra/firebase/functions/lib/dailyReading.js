...
async function getDailyReading(data, context, geminiApiKey) {
    ...
    if (!todayReading.empty) {
        ...
        const hasHoroscopeInForbiddenFields = checkForHoroscope(existingReading.main_text || "") ||
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            console.warn(`Daily reading Gemini API error (retryable), retrying in ${delay}ms:`, error?.message || "unknown");
            // Wait before retry
...
function parseDailyReadingResponse(text, date) {
    ...
    try {
        ...
        // Provjeri sve polja koja NE SMIJU imati horoskopski jezik
        const hasHoroscopeInForbiddenFields = checkForHoroscope(parsed.main_text || "") ||
        ...
        const filterCupReferences = (text) => {
            if (!text)
                return text;
            return text
